...
DIST_PATH = create_bigram_dist_folder() # Присваиваем абсолютный путь к папке переменной DIST_PATH
...
def calculate_bigram_entropy(bigram_alphabet) -> float | int:
    ...
    probabilities: list[float] = [freq / total_bigrams for freq in bigram_alphabet.values()]
...
def encode_text_bigrams(text: str, codes: dict[str, str]) -> str:
    ...
    if len(text) % 2 == 1:
        ...
        print(f"Примечание: Последний символ '{last_char}' не является частью биграммы и не закодирован.")
...
def decode_text_bigrams(encoded_text: str, codes: dict[str, str]) -> str:
    ...
    if code_buffer:
        print(f"Предупреждение: Неполная последовательность кода '{code_buffer}' найдена в конце кодированного текста при декодировании.")
...
def main() -> None:
    ...
    write_bigram_codes_to_csv(shannon_fano_bigram_codes, 'shannon_fano_bigram_codes.csv')

    avg_bigram_code_length: float = calculate_average_code_length(shannon_fano_bigram_codes, bigram_alphabet)
    ...
    print(f'Энтропия буквосочетаний: {bigram_entropy}')
    print(f'Средняя длина буквосочетаний: {avg_bigram_code_length}')
    print(f'Эффективность сжатия буквосочетаний: {compression_efficiency_bigram}')

    # Кодируем текст с использованием кодов биграмм
    try:
        encoded_bigram_text: Any | Literal[''] = encode_text_bigrams(text, shannon_fano_bigram_codes)
    ...
    decoded_bigram_text: Any | Literal[''] = decode_text_bigrams(encoded_bigram_text, shannon_fano_bigram_codes)
